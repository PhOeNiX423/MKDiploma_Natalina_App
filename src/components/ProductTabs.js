...
const ProductTabs = ({ product }) => {
  ...
  useEffect(() => {
    const fetchReviews = async () => {
      try {
        const res = await fetch(`${process.env.REACT_APP_DB_URL_REVIEWS}/${productId}`);
      ...
      <div className="mt-8">
        {activeTab === 'Отзывы' && (
          ...
          <div className="flex flex-col md:flex-row gap-8">
            ...
            <div className="md:w-1/3 text-center md:text-left">
              ...
              <p className="text-sm text-secondary mt-2">({totalReviews} отзывов)</p>
            </div>
                ...
                <div key={star} className="flex items-center gap-2">
                  ...
                  <div className="flex-1 bg-gray-200 rounded-full h-2">
                    <div
                      ...
                      style={{ width: `${getPercent(reviewsBreakdown[star])}%` }}
                    />
                  </div>
                </div>
              ))}
          ...
          <ProductReviews productId={product._id} visible={activeTab === 'Review'} />
        </>
        )}
        ...
        {activeTab === 'Детали' && (
          <div className="text-secondary mt-4 space-y-2">
            ...
            {target_area && (
              <p>
                <span className="font-semibold text-primary">Зона применения: </span>
                {target_area}
              </p>
            )}
            {item_code && (
